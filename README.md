...
(starting with `-100`). A helper script is included to find numeric IDs:

```bash
python list_ids.py
```

On first run you'll be asked for your phone number and the code sent to you
(and your 2FA password if enabled). The script lists every **group** you're a
member of, along with its ID. Use these values in your `.env` file.
